...
(function (root) {
  ...
  var PARAMS = [
    ...
    { key: 'follow',    label: 'Follow',   min: 0, max: 1, step: 0.01, def: 0,
      hint: "Retune the input into the detected chord (read from the codec's own allocation). At 1 the out-of-chord partials move; below, the original and pulled pitch sound together." },
    { key: 'followMode', label: 'Pull',    def: 'partial', options: ['partial', 'frame'],
      hint: 'partial: every prominent partial snaps to the nearest chord tone, harmonic stacks moving together. frame: the dominant partial sets one interval and the whole frame is transposed by it.' },
